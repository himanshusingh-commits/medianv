...
export class socketGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  broadcastMessage(user: string, message: string) {
    this.server.emit('room', `[${user}] -> ${message}`);
  }
  ...
  async handleMessage(
    ...
    console.log(`Message from ${client.id} (${userName}): ${content}`);
    await this.messageService.saveMessage(userName, content);
    this.server.emit('room', `[${userName}] -> ${content}`);
  }
}
